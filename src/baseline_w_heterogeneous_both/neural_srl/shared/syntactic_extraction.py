...
class SyntacticCONLL(object):
    ...
    def read_from_file(self, filename):
        ...
        for i, line in enumerate(conll_file.readlines()):
            if line == '\n' or line == '\r\n':  # new sentence
                sentence_id += 1
                if a_tree.heads.count(-1) != 1:
                    print("line number", i, "sentence number", len(self.trees) + 1)
                assert a_tree.heads.count(-1) == 1
            ...
            # dependency tree heads probabilities
            phead = torch.max(torch.FloatTensor([float(item) for item in tokens[8][1:-1].split(", ")]))
            a_tree.heads_probs.append(phead)
    ...
    def get_tpf2_dict(self, corpus_tensors, tpf2_dict=None):
        ...
        for tensor in corpus_tensors:
            ...
            print "the TPF module doesn't appropriate this framework"
            exit()
    ...
    def get_batch_TreeGRU_data(self, sentence_ids, sentence_lengths):
        batch_size, max_sen_length = len(sentence_ids), max(sentence_lengths)
        dep_labels_ids = Variable(torch.LongTensor(batch_size, max_sen_length).zero_(), requires_grad=False)
        heads, pheads = [], []

        for i, sen_id in enumerate(sentence_ids):
            focused_tree = self.trees[sen_id]
            assert len(focused_tree.labels_id) == sentence_lengths[i]
            for j in range(sentence_lengths[i]):
                # print dep_labels_ids.size(), focused_tree.labels_id[j]
                dep_labels_ids[i, j] = focused_tree.labels_id[j]
            heads.append(copy.deepcopy(focused_tree.heads))
            pheads.append(focused_tree.heads_probs)
        return dep_labels_ids, (heads, pheads)
