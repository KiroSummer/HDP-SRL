...
class TaggerData(object):
    def __init__(self, config, train_sents, dev_sents, dep_datas, eval_data, word_dict, head_dict, char_dict,
                 label_dict, dep_label_dicts,
        ...
        # dependency data
        self.dep_treebanks = dep_datas
        self.dep_treebanks_number = len(self.dep_treebanks)
        self.dep_label_dicts = dep_label_dicts
    ...
    def get_dep_training_data(self, include_last_batch=False):
        ...
        """
        result_deps = []
        for i in xrange(self.dep_treebanks_number):
            dep_data = self.dep_treebanks[i].sample_dep_data
            random.shuffle(dep_data)
            results, batch_tensors = [], []
            batch_num_tokens = 0
            for i, example in enumerate(dep_data):
            ...
            print("Extracted {} samples and {} batches.".format(len(dep_data), len(results)))
            result_deps.append(results)
        return result_deps
    ...
    def mix_training_data(srl_data, dep_data):
        dep_treebanks_number = len(dep_data)
        max_dep_batch_size = max([len(dep_t) for dep_t in dep_data])
        print([(max_dep_batch_size, len(dep_t)) for dep_t in dep_data])
        zipped_dep_treebanks = zip(*[dep_t * int(math.ceil(max_dep_batch_size * 1.0 / len(dep_t))) for dep_t in dep_data])
        print("zipped dep treebanks size", len(zipped_dep_treebanks))
        max_batch_size = max(len(srl_data), max_dep_batch_size)
        min_dep_batch_size = min([len(dep_t) for dep_t in dep_data])
        min_batch_size = min(len(srl_data), min_dep_batch_size)
        multiple = int(max_batch_size / min_batch_size)
        print("max dep batch size is", max_dep_batch_size, " ; min dep batch size", min_dep_batch_size)
        # ok, i know the dep_data is small_size
        which_bigger = 0 if len(srl_data) > min_dep_batch_size else 1
        if which_bigger == 0:
            # it's impossible
            results = zip(srl_data, zipped_dep_treebanks * multiple)  # the order is sensitive
            print("After mix operation, the resulting training batch size is {}".format(len(results)))
            return results
        else:
            results = zip(srl_data * multiple, zipped_dep_treebanks)  # the order is sensitive
            print("After mix operation, the resulting training batch size is {}".format(len(results)))
            return results
...
def get_hete_dep_trees_info(hete_dep_trees, sent_ids, sent_lengths, training=True, is_cuda=True):
    hete_dep_trees_info = []
    for i, dep_treebank in enumerate(hete_dep_trees):
        dep_treebank = dep_treebank[0] if training else dep_treebank[1]
        dep_label_ids, heads = dep_treebank.get_batch_TreeGRU_data(sent_ids, sent_lengths)
        if is_cuda:
            dep_label_ids = dep_label_ids.cuda()
        hete_dep_trees_info.append((dep_label_ids, heads))
    return hete_dep_trees_info


def get_predicted_hete_dep_trees_info(hete_dep_trees, sent_ids, sent_lengths, is_cuda=True):
    hete_dep_trees_info = []
    for i, dep_treebank in enumerate(hete_dep_trees):
        dep_label_ids, heads = dep_treebank.get_batch_TreeGRU_data(sent_ids, sent_lengths)
        if is_cuda:
