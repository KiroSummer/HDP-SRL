...
from ..gcn_model.various_gcn import HeterProbasedGraphConvLayerWRNN
from ..gcn_model.tree import Tree
from ..gcn_model.gcn import *
from module import BiaffineScorer
...
class BiLSTMTaggerModel(nn.Module):
    ...
    def __init__(self, data, config, gpu_id=""):
        ...
        self.syntactic_label_dim = config.syn_label_dim
        self.cuda_id = gpu_id
        # dependency
        self.dep_treebank_number = len(data.dep_label_dicts)
        self.dep_label_space_sizes = []
        for i in xrange(self.dep_treebank_number):
            self.dep_label_space_sizes.append(data.dep_label_dicts[i].size())
        ...
        # syntactic label embedding
        self.hete_dep_label_embeddings = []
        for i in range(self.dep_treebank_number):
            syntactic_label_embedding = nn.Embedding(self.dep_label_space_sizes[i], self.syntactic_label_dim)
            self.hete_dep_label_embeddings.append(syntactic_label_embedding)
        self.hete_dep_label_embeddings = nn.ModuleList(self.hete_dep_label_embeddings)
        ...
        # word emb dim + char cnn dim
        self.input_size = self.word_embedding_dim + 3 * self.config.output_channel
        # Initialize HighwayBiLSTM
        self.lstm_input_size = self.input_size + self.config.gcn_hidden_dim
        ...
        # GCN
        # self.gcn = GCN(config, self.input_size, config.gcn_hidden_dim, config.gcn_num_layers)
        self.gcn = HeterProbasedGraphConvLayerWRNN(config, self.input_size, config.gcn_hidden_dim,
                                                   config.gcn_num_layers, self.dep_treebank_number)
        ...
        self.biaffine_scorers = []
        for i in xrange(self.dep_treebank_number):
            self.biaffine_scorers.append(BiaffineScorer(2 * self.lstm_hidden_size, self.config.mlp_arc_size,
                                                  self.config.mlp_rel_size, self.dep_label_space_sizes[i], config))
        self.biaffine_scorers = nn.ModuleList(self.biaffine_scorers)

        self.reset_parameters()

    def reset_parameters(self):
        init.xavier_uniform_(self.char_embedding.weight)
        for emb in self.hete_dep_label_embeddings:
    ...
    def get_span_emb(self, flatted_head_emb, flatted_context_emb, flatted_candidate_starts, flatted_candidate_ends,
        ...
        span_indices_log_mask = self.sequence_mask(span_width, max_sent_length=max_arg_width)
        span_indices_log_mask = torch.log(span_indices_log_mask.type(torch.Tensor))
    ...
    def forward(self, sent_lengths, words, heads, chars, labels, gold_predicates=None, tree_gru_input=None, dep=None):
        ...
        if dep is not None:
            ...
            loss = self.biaffine_scorers[dep[0]].forward(lstm_out, dep[1:], sent_lengths)
            return loss
        ...
        hete_heads = [item[1] for item in tree_gru_input]
        gcn_output = self.gcn.forward(hete_heads, words, context_embeddings, masks, maxlen=max_sent_length)
        """GCN end"""
        # dep lstm
        ...
        dep_representations = self.gamma * (
                    normed_weights[0] * dep_lstm_outputs[0] + normed_weights[1] * dep_lstm_outputs[1]
                    + normed_weights[2] * dep_lstm_outputs[2])
        context_embeddings = torch.cat([context_embeddings, gcn_output, dep_representations], dim=-1)
    ...
    def load(self, filepath):
        ...
        """
        # model_params = self.load_state_dict(torch.load(filepath))
